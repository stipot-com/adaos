...
export function installTelegramWebhookRoutes(app: express.Express, bus: NatsBus | null) {
  app.post('/io/tg/:bot_id/webhook', async (req, res) => {
    try {
      ...
      if (token) {
        try {
          ...
          if (evt.type === 'audio' && evt.payload?.file_id) {
            ...
            const fpath = typeof info === 'string' ? info : info?.file_path
            ...
            if (fsz && fsz > maxMb * 1024 * 1024) {
              ...
              if (fpath) {
                ...
                evt.payload.audio_path = wav || local
              }
            ...
            if (fsz && fsz > maxMb * 1024 * 1024) {
              log.warn({ bot_id, update_id: evt.update_id, file_size: fsz }, 'document too large, skip download')
                ...
                const local = await downloadFile(token, fpath, bot_id)
                evt.payload.document_path = local
            }
          }
        } catch { /* non-fatal */ }
      }
        ...
        if (code) {
          ...
          if (rs) {
            try {
              ...
              const resp = await request(url, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ code, user_id: evt.user_id, bot_id }), dispatcher })
      ...
      // resolve hub
      const locale = evt.payload?.meta?.lang
      ...
      if (bus) { try { await bus.publish_dlq('input', { error: String(e) }) } catch {} }
