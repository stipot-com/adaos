...
export type WebSessionStage = 'NEW' | 'PREAUTH' | 'WEBREG' | 'AUTH' | 'PAIRED' | 'ONLINE'
...
export interface WebAuthnService {
  verifyDeviceCodeLogin(deviceCodeOrUserCode: string, sid: string): Promise<{ ok: boolean; owner_id?: string; subnet_id?: string; hub_id?: string; error?: 'invalid_device_code' | 'expired_token' }>
  createRegistrationChallenge(sid: string): Promise<{ ok: boolean; publicKeyCredentialCreationOptions?: any; error?: 'session_not_found' | 'invalid_state' }>
  finishRegistration(sid: string, credential: any): Promise<{ ok: boolean; browser_key_id?: string; error?: 'session_not_found' | 'invalid_state' | 'verification_failed' }>
  createLoginChallenge(sid: string): Promise<{ ok: boolean; publicKeyCredentialRequestOptions?: any; error?: 'session_not_found' | 'registration_required' }>
  finishLogin(sid: string, credential: any): Promise<{ ok: boolean; session_jwt?: string; browser_key_id?: string; error?: 'session_not_found' | 'assertion_failed' }>
}
...
async function loadSession(redis: RedisClientType<any, any, any>, sid: string): Promise<WebSessionState | null> {
...
async function saveSession(redis: RedisClientType<any, any, any>, session: WebSessionState): Promise<void> {
...
async function findDeviceCodeByUserOrDevice(redis: RedisClientType<any, any, any>, code: string): Promise<DeviceCodeRecord | null> {
  ...
  const iterator = redis.scanIterator({ MATCH: 'device_code:*', COUNT: 100 }) as AsyncIterable<string>
...
export async function storeDeviceCode(
  ...
  await redis.setEx(deviceCodeKey(record.device_code), ttl, JSON.stringify(record))
}
...
export function createWebAuthnService(deps: WebAuthnDeps): WebAuthnService {
  ...
  return {
    async verifyDeviceCodeLogin(deviceCodeOrUserCode: string, sid: string) {
      const rec = await findDeviceCodeByUserOrDevice(redis, deviceCodeOrUserCode)
      if (!rec) {
        return { ok: false as const, error: 'invalid_device_code' as const }
      }
      ...
      rec.bind_sid = sid
      await redis.setEx(deviceCodeKey(rec.device_code), Math.max(30, rec.exp - nowSeconds()), JSON.stringify(rec))
    ...
    async finishRegistration(sid: string, credential: any) {
      const session = await loadSession(redis, sid) as (WebSessionState & { webreg_challenge?: string }) | null
      if (!session) {
        return { ok: false as const, error: 'session_not_found' as const }
      }
    ...
    async createLoginChallenge(sid: string) {
      ...
      if (!session.browser_key_id) {
        return { ok: false as const, error: 'registration_required' as const }
      }
    ...
    async finishLogin(sid: string, credential: any) {
      const session = await loadSession(redis, sid) as (WebSessionState & { weblogin_challenge?: string }) | null
      if (!session) {
        return { ok: false as const, error: 'session_not_found' as const }
      }
      if (!session.browser_key_id) {
        return { ok: false as const, error: 'assertion_failed' as const }
      }
      ...
      try {
        ...
        if (recordRaw) {
          try {
            const rec = JSON.parse(recordRaw) as WebAuthnCredentialRecord
            rec.sign_count += 1
            await redis.set(webAuthnCredKey(credId), JSON.stringify(rec))
          } catch {
            // ignore
          }
        }
        ...
        await redis.setEx(`session:jwt:${session_jwt}`, defaultSessionTtlSeconds, JSON.stringify({ sid, owner_id: session.owner_id, browser_key_id: session.browser_key_id }))

        return { ok: true as const, session_jwt, browser_key_id: session.browser_key_id }
      } catch {
        return { ok: false as const, error: 'assertion_failed' as const }
      }
    },
...
export type ErrorResponder = (req: express.Request, res: express.Response, status: number, code: string, params?: Record<string, any>) => void
...
export function installWebAuthnRoutes(
  ...
  app.post('/v1/owner/login/verify', async (req, res) => {
    try {
      const sid = extractSid(req.body)
      const code = extractDeviceCode(req.body)
      if (!sid) {
        return respondError(req, res, 400, 'missing_field', { field: 'sid' })
      }
      if (!code) {
        return respondError(req, res, 400, 'missing_field', { field: 'device_code' })
      }
      const result = await service.verifyDeviceCodeLogin(code, sid)
      if (!result.ok) {
        const status = result.error === 'invalid_device_code' ? 400 : 400
  ...
  app.post('/v1/owner/webauthn/registration/challenge', async (req, res) => {
    try {
      const sid = extractSid(req.body)
      if (!sid) {
        return respondError(req, res, 400, 'missing_field', { field: 'sid' })
      ...
      if (!result.ok) {
        const code = result.error === 'session_not_found' ? 'invalid_device_code' : 'invalid_request'
        return respondError(req, res, 400, code)
      }
      res.json({ publicKeyCredentialCreationOptions: result.publicKeyCredentialCreationOptions })
    } catch (error) {
  ...
  app.post('/v1/owner/webauthn/registration/finish', async (req, res) => {
    try {
      const sid = extractSid(req.body)
      ...
      if (!credential) {
        return respondError(req, res, 400, 'missing_field', { field: 'credential' })
      }
  ...
  app.post('/v1/owner/webauthn/login/challenge', async (req, res) => {
    try {
      const sid = extractSid(req.body)
      if (!sid) {
        return respondError(req, res, 400, 'missing_field', { field: 'sid' })
      ...
      if (!result.ok) {
        const code = result.error === 'registration_required' ? 'invalid_device_code' : 'invalid_request'
        return respondError(req, res, 400, code)
      }
      res.json({ publicKeyCredentialRequestOptions: result.publicKeyCredentialRequestOptions })
    } catch (error) {
  ...
  app.post('/v1/owner/webauthn/login/finish', async (req, res) => {
    try {
      const sid = extractSid(req.body)
      const credential = (req.body as any)?.credential
      if (!sid) {
      ...
      res.json({ session_jwt: result.session_jwt, browser_key_id: result.browser_key_id })
    } catch (error) {
