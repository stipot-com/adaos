...
def put(key: str, value: Any, ttl: int | None = None) -> None:
    ...
    qualified = _qualified_key(key)
    if ttl is not None and hasattr(ctx.kv, "set"):
        try:
            ctx.kv.set(qualified, value, ttl=ttl)  # type: ignore[call-arg]
            return
        except TypeError:
            pass
    ctx.kv.set(qualified, value)
