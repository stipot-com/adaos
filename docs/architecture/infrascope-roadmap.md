...
- supervisor-backed operator projections now also carry scheduled/deferred core-update state, queued follow-up transitions, countdown defer actions, and warm-switch admission context (`transition_mode`, candidate runtime URL/port, fallback reason) so operators can see not only when an update is planned but also whether supervisor intends a warm candidate handoff or a stop-and-switch fallback
