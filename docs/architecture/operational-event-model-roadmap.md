...
- sidecar rollout policy is now also explicit through shared reliability/browser surfaces, so hub `default-on` transport adoption can be audited separately from deeper post-Phase-0 continuity and session-runtime work
