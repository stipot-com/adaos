...
- [ ] `phase0.node_browser_ready`: in progress; Realtime Reliability now treats browser/member semantic channels and `Yjs as SyncChannel` as complete for the current scope, and the remaining `/yws` blocker is narrowed to final public ownership cutover because `adaos-realtime` now exposes a ready local `/yws` proxy listener through the shared `event_model_phase0_communication` checkpoint
- [ ] `phase0.runtime_comm_ready`: in progress; hub-root Class A hardening, browser-safe supervisor transition state, and routed-browser active-runtime selection are now explicit through shared reliability runtime surfaces, optional sidecar continuity no longer blocks the prerequisite by default, and local `/ws` sidecar proxy readiness is now real, but final `/ws` ownership cutover still keeps the runtime communication prerequisite open
...
- sidecar rollout policy is now also explicit through shared reliability/browser surfaces, so hub `default-on` transport adoption can be audited separately from the still-open `/ws` and `/yws` ownership blockers
- `adaos-realtime` now boots dedicated local `/ws` and `/yws` proxy listeners and shared reliability/browser surfaces mark them as `proxy_ready`, so the remaining Phase 0 communication blocker is the public/browser ingress cutover rather than missing sidecar listener scaffolding
- for Event Model `Phase 0`, the remaining runtime communication blocker is now the still-runtime-owned `/ws` transport boundary; supervisor continuity remains a subordinate Realtime Reliability hardening track, but no longer a hidden visibility gap
