...
- [ ] `phase0.runtime_comm_ready`: in progress; hub-root Class A hardening, browser-safe supervisor transition state, and routed-browser active-runtime selection are now explicit through shared reliability runtime surfaces, and optional sidecar continuity no longer blocks the prerequisite by default, but sidecar `/ws` ownership expansion still keeps the runtime communication prerequisite open
