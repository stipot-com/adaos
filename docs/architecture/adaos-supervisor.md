...
- operator tooling can already bridge some local-only cases by combining root-published fetch metadata with a direct pull from the current hub supervisor control surface, without pretending that heavy artifacts are replicated at root
