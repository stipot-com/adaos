...
The remaining gap is full candidate-runtime prewarm and fast cutover: the browser/control path can now see which mode supervisor selected and which candidate URL/port is reserved, root-facing identity is ready for parallel runtimes, and slot preparation can already happen before outage, but the actual dual-runtime launch/cutover path still needs to be completed so low-downtime switching becomes operational rather than only planned/observable.
